...
namespace QuanLySieuThi.Data
{
    public class SalesRepository
    {
        ...
        private string MapVoucherToLine(Voucher vc)
        {
            ...
            // Gốc của Voucher (Dùng chung cho cả 2 loại)
            string baseData = $"VC|{vc.MaVoucher}|{vc.TenVoucher}|{nbd}|{nkt}|{vc.DKApDung}|{vc.TrangThai}";

            // Kiểm tra xem là loại Voucher nào để lấy dữ liệu riêng
            if (vc is VoucherPhanTram vp)
            {
                return baseData + $"|phần trăm|{vp.PhanTramGiam}|{vp.GiamToiDa}";
            }
            else if (vc is VoucherTienMat vt)
            {
                return baseData + $"|tiền mặt|{vt.SoTienGiamCoDinh}|0";
            }
