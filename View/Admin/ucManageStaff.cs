...
namespace ChuongtrinhQuanlybanhangsieuthi.View
{
    public partial class ucManageStaff : UserControl
    {
        ...
        private void HienThiLenBang()
        {
            ...
            if (dgvNhanVien.Columns["Taikhoan"] != null) dgvNhanVien.Columns["Taikhoan"].Visible = false;
            if (dgvNhanVien.Columns["GioiTinh"] != null) dgvNhanVien.Columns["GioiTinh"].Visible = false;

            if (dgvNhanVien.Columns["HienThiGioiTinh"] != null)
                dgvNhanVien.Columns["HienThiGioiTinh"].HeaderText = "Giới tính";

            if (dgvNhanVien.Columns["Ma"] != null) dgvNhanVien.Columns["Ma"].Visible = false;
            if (dgvNhanVien.Columns["MaCa"] != null) dgvNhanVien.Columns["MaCa"].Visible = false;
        }
        ...
        private void dgvNhanVien_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = dgvNhanVien.Rows[e.RowIndex]; // Đã sửa lỗi dùng var
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                serviceNhanSu.LuuDuLieuNhanSu();

                HienThiLenBang();
                MessageBox.Show("Đã thêm nhân viên và lưu file thành công!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            if (dgvNhanVien.CurrentRow != null)
            {
                ...
                btnLuu.Enabled = true;
                MessageBox.Show("Mời bạn chỉnh sửa thông tin phía bên trái, sau đó bấm 'Lưu'.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("Vui lòng chọn một nhân viên từ bảng để sửa!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (thanhCong)
                {
                    ...
                    MessageBox.Show("Đã lưu thay đổi vào cơ sở dữ liệu!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                {
                    MessageBox.Show("Không tìm thấy mã nhân viên này trong hệ thống!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void txtTim_TextChanged(object sender, EventArgs e)
        {
            ...
            List<NhanVien> ketQuaTimKiem = serviceNhanSu.TimKiemNhanVien(keyword);
