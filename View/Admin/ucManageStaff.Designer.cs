namespace ChuongtrinhQuanlybanhangsieuthi.View
{
    partial class ucManageStaff
    {
        /// <summary> 
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary> 
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Component Designer generated code

        /// <summary> 
        /// Required method for Designer support - do not modify 
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            groupBox1 = new GroupBox();
            richTextBox1 = new RichTextBox();
            label6 = new Label();
            rtbDiaChi = new RichTextBox();
            label5 = new Label();
            rtbChucVu = new RichTextBox();
            label4 = new Label();
            rtbSDT = new RichTextBox();
            rtbHoTen = new RichTextBox();
            rtbMaNV = new RichTextBox();
            label3 = new Label();
            label2 = new Label();
            label1 = new Label();
            dgvNhanVien = new DataGridView();
            btnThem = new Button();
            btnSua = new Button();
            btnXoa = new Button();
            btnLuu = new Button();
            groupBox1.SuspendLayout();
            ((System.ComponentModel.ISupportInitialize)dgvNhanVien).BeginInit();
            SuspendLayout();
            // 
            // groupBox1
            // 
            groupBox1.BackColor = SystemColors.ActiveBorder;
            groupBox1.Controls.Add(richTextBox1);
            groupBox1.Controls.Add(label6);
            groupBox1.Controls.Add(rtbDiaChi);
            groupBox1.Controls.Add(label5);
            groupBox1.Controls.Add(rtbChucVu);
            groupBox1.Controls.Add(label4);
            groupBox1.Controls.Add(rtbSDT);
            groupBox1.Controls.Add(rtbHoTen);
            groupBox1.Controls.Add(rtbMaNV);
            groupBox1.Controls.Add(label3);
            groupBox1.Controls.Add(label2);
            groupBox1.Controls.Add(label1);
            groupBox1.Font = new Font("Segoe UI", 12F, FontStyle.Regular, GraphicsUnit.Point, 0);
            groupBox1.Location = new Point(110, 38);
            groupBox1.Name = "groupBox1";
            groupBox1.Size = new Size(877, 162);
            groupBox1.TabIndex = 0;
            groupBox1.TabStop = false;
            groupBox1.Text = "Thông tin";
            groupBox1.Enter += groupBox1_Enter;
            // 
            // richTextBox1
            // 
            richTextBox1.Location = new Point(427, 84);
            richTextBox1.Name = "richTextBox1";
            richTextBox1.Size = new Size(188, 27);
            richTextBox1.TabIndex = 11;
            richTextBox1.Text = "";
            // 
            // label6
            // 
            label6.AutoSize = true;
            label6.Font = new Font("Segoe UI", 10.8F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label6.Location = new Point(339, 84);
            label6.Name = "label6";
            label6.Size = new Size(82, 25);
            label6.TabIndex = 10;
            label6.Text = "Giới tính:";
            label6.Click += label6_Click;
            // 
            // rtbDiaChi
            // 
            rtbDiaChi.Location = new Point(397, 117);
            rtbDiaChi.Name = "rtbDiaChi";
            rtbDiaChi.Size = new Size(218, 27);
            rtbDiaChi.TabIndex = 9;
            rtbDiaChi.Text = "";
            // 
            // label5
            // 
            label5.AutoSize = true;
            label5.Font = new Font("Segoe UI", 10.8F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label5.Location = new Point(322, 117);
            label5.Name = "label5";
            label5.Size = new Size(69, 25);
            label5.TabIndex = 8;
            label5.Text = "Địa chỉ:";
            // 
            // rtbChucVu
            // 
            rtbChucVu.Location = new Point(92, 84);
            rtbChucVu.Name = "rtbChucVu";
            rtbChucVu.Size = new Size(215, 27);
            rtbChucVu.TabIndex = 7;
            rtbChucVu.Text = "";
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.Font = new Font("Segoe UI", 10.8F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label4.Location = new Point(6, 84);
            label4.Name = "label4";
            label4.Size = new Size(80, 25);
            label4.TabIndex = 6;
            label4.Text = "Chức vụ:";
            // 
            // rtbSDT
            // 
            rtbSDT.Location = new Point(60, 117);
            rtbSDT.Name = "rtbSDT";
            rtbSDT.Size = new Size(247, 27);
            rtbSDT.TabIndex = 5;
            rtbSDT.Text = "";
            // 
            // rtbHoTen
            // 
            rtbHoTen.Location = new Point(339, 45);
            rtbHoTen.Name = "rtbHoTen";
            rtbHoTen.Size = new Size(276, 27);
            rtbHoTen.TabIndex = 4;
            rtbHoTen.Text = "";
            // 
            // rtbMaNV
            // 
            rtbMaNV.BackColor = SystemColors.Window;
            rtbMaNV.Location = new Point(82, 45);
            rtbMaNV.Name = "rtbMaNV";
            rtbMaNV.Size = new Size(125, 27);
            rtbMaNV.TabIndex = 3;
            rtbMaNV.Text = "";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.Font = new Font("Segoe UI", 10.8F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label3.Location = new Point(6, 117);
            label3.Name = "label3";
            label3.Size = new Size(48, 25);
            label3.TabIndex = 2;
            label3.Text = "SĐT:";
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.Font = new Font("Segoe UI", 10.8F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label2.Location = new Point(6, 45);
            label2.Name = "label2";
            label2.Size = new Size(70, 25);
            label2.TabIndex = 1;
            label2.Text = "Mã NV:";
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Segoe UI", 10.8F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label1.Location = new Point(263, 45);
            label1.Name = "label1";
            label1.Size = new Size(70, 25);
            label1.TabIndex = 0;
            label1.Text = "Họ tên:";
            label1.Click += label1_Click;
            // 
            // dgvNhanVien
            // 
            dgvNhanVien.ColumnHeadersHeightSizeMode = DataGridViewColumnHeadersHeightSizeMode.AutoSize;
            dgvNhanVien.Location = new Point(17, 217);
            dgvNhanVien.Name = "dgvNhanVien";
            dgvNhanVien.RowHeadersWidth = 51;
            dgvNhanVien.Size = new Size(1167, 389);
            dgvNhanVien.TabIndex = 1;
            // 
            // btnThem
            // 
            btnThem.BackColor = SystemColors.ActiveCaption;
            btnThem.ForeColor = SystemColors.ControlText;
            btnThem.Location = new Point(1006, 38);
            btnThem.Name = "btnThem";
            btnThem.Size = new Size(94, 36);
            btnThem.TabIndex = 2;
            btnThem.Text = "Thêm";
            btnThem.UseVisualStyleBackColor = false;
            btnThem.Click += btnThem_Click;
            // 
            // btnSua
            // 
            btnSua.BackColor = SystemColors.ActiveCaption;
            btnSua.Location = new Point(1006, 122);
            btnSua.Name = "btnSua";
            btnSua.Size = new Size(94, 36);
            btnSua.TabIndex = 3;
            btnSua.Text = "Sửa";
            btnSua.UseVisualStyleBackColor = false;
            // 
            // btnXoa
            // 
            btnXoa.BackColor = SystemColors.ActiveCaption;
            btnXoa.Location = new Point(1006, 80);
            btnXoa.Name = "btnXoa";
            btnXoa.Size = new Size(94, 36);
            btnXoa.TabIndex = 4;
            btnXoa.Text = "Xóa";
            btnXoa.UseVisualStyleBackColor = false;
            // 
            // btnLuu
            // 
            btnLuu.BackColor = SystemColors.ActiveCaption;
            btnLuu.Location = new Point(1006, 164);
            btnLuu.Name = "btnLuu";
            btnLuu.Size = new Size(94, 36);
            btnLuu.TabIndex = 5;
            btnLuu.Text = "Lưu";
            btnLuu.UseVisualStyleBackColor = false;
            // 
            // ucManageStaff
            // 
            AutoScaleDimensions = new SizeF(8F, 20F);
            AutoScaleMode = AutoScaleMode.Font;
            Controls.Add(btnLuu);
            Controls.Add(btnXoa);
            Controls.Add(btnSua);
            Controls.Add(btnThem);
            Controls.Add(dgvNhanVien);
            Controls.Add(groupBox1);
            Name = "ucManageStaff";
            Size = new Size(1202, 676);
            Load += ucManageStaff_Load;
            groupBox1.ResumeLayout(false);
            groupBox1.PerformLayout();
            ((System.ComponentModel.ISupportInitialize)dgvNhanVien).EndInit();
            ResumeLayout(false);
        }

        #endregion

        private GroupBox groupBox1;
        private Label label1;
        private Label label2;
        private Label label3;
        private RichTextBox rtbSDT;
        private RichTextBox rtbHoTen;
        private RichTextBox rtbMaNV;
        private Label label4;
        private RichTextBox rtbChucVu;
        private DataGridView dgvNhanVien;
        private Button btnThem;
        private Label label6;
        private RichTextBox rtbDiaChi;
        private Label label5;
        private RichTextBox richTextBox1;
        private Button btnSua;
        private Button btnXoa;
        private Button btnLuu;
    }
}
